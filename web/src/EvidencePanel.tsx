...
export default function EvidencePanel({state,action,addNote,busy}:{state:GameState;action:(body:any)=>Promise<void>;addNote:(e:Evidence)=>void;busy:boolean}){
 ...
 return <div className="side-panel evidence-panel">
  <div className="panel-head"><span>{state.evidence.length} 件已发现</span><small>选中证物可分析或出示</small></div>
  {!state.evidence.length&&<div className="empty-small">调查场景中的对象，第一件证物就在那里。</div>}
  <div className="evidence-list">{state.evidence.map(e=><button className={`evidence-item ${selectedId===e.id?'selected':''}`} key={e.id} onClick={()=>setSelectedId(e.id)}><span className="evidence-type">{e.type==='document'?'文':e.type==='trace'?'痕':e.type==='deduction'?'链':'物'}</span><span><b>{e.title}</b><small>{e.source} · {e.time}</small></span>{e.analyzed&&<ShieldCheck size={14} aria-label="已分析"/>}</button>)}</div>
