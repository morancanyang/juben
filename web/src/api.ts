...
export type Character = {id:string;name:string;role:string;bio:string;color:string;trust:number;alertness:number;emotion:string;suggestions:string[]}
...
export async function clearLocal(){await Promise.all([localDB.drafts.clear(),localDB.snapshots.clear(),localDB.intents.clear()]);for(const k of Object.keys(localStorage))if(k.startsWith('casebook_'))localStorage.removeItem(k)}
